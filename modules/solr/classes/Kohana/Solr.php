...
class Kohana_Solr {
	 ...
	 * Servlet mappings
	 */
	const UPDATE_SERVLET = 'update/json?commit=true';
	...
	public static function instance($name = 'default', array $config = NULL)
	{
		if ( ! isset(Solr::$instances[$name]))
		{
			if ($config === NULL)
			{
				$config = Kohana::$config->load('solr')->$name;
	...
	public function index(array $document, $overwrite = TRUE, $commit_within = FALSE, $boost = NULL)
	{
		...
		{ 
		    function filter_nulls($v) { return $v !== NULL; };
		}
		
		$document = array_filter($document, 'filter_nulls');
	...
	public function request($data, array $params = array())
	{
		...
		$request = Request::factory($url)->method('POST');
		...
		$log->add(Log::DEBUG,"Response from SOLR:".serialize($response)." \n");
