...
var SendOrder = function () {
    ...
    return {
        ...
        init: function () {
        	...
        	$('select[name=shipmentcompany_id]').change(function(){
        		
        		if($(this).val() > 0){
        		
        			if($('select[name=shipmentcompany_id] option[value='+$(this).val()+']').html() == 'DPD') {
        				$('input[name=shipping_number]').rules('remove','required');
        				$('input[name=shipping_number]').parents('.form-group').hide();
        	        
        			}else {
        				$('input[name=shipping_number]').rules('add','required');
        				$('input[name=shipping_number]').parents('.form-group').show();
        			}
        		}
        	});
