...
class DpdApi {
    ...
    public function setConnection() {
        ...
        try {
            ...
            $config_options=$_config->options;
    ...
    private function saveFile($folder, $content, $filename){
        ...
        $protocols_dir = $this->baseFolder.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$this->current_year.DIRECTORY_SEPARATOR.$this->current_month.DIRECTORY_SEPARATOR;
        
        if(!is_dir($protocols_dir)) mkdir($protocols_dir,"0770",TRUE);
        
        $file = $protocols_dir.$filename.".pdf";

        if(file_put_contents($file, base64_decode($content))!=false) {
            return DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$this->current_year.DIRECTORY_SEPARATOR.$this->current_month.DIRECTORY_SEPARATOR.$filename.'.pdf';
        } else {
            return false;
        }
    }
    ...
    public function getProtocol(Array $references) {
        ...
        try {
            ...
            if($founded_all and $xml->DocumentId!='') {
                $filename = $this->saveFile("protocols", $xml->DocumentData, $this->department.'_'.$xml->DocumentId);
    ...
    public function registerNewPackage(){
        ...
        try {
            ...
            $params1->policyV1="STOP_ON_FIRST_ERROR";
            ...
            switch($xml->Status) {
                ...
                case "INCORRECT_DATA":
                    ...
                    foreach($xml->Packages->Package->InvalidFields->InvalidField as $invalid_field) {
                        if($invalid_field->Status == "VALUE_INCORRECT" || $invalid_field->Status == "DONT_MATCH_PATTERN") {
    ...
    public function getLabelPDF($findBy, $string) {
        /*
         * zapisz etykiete w PDF, do podanego katalogu
         * findBy: 1 - po reference, 2 - po waybill
         */
        ...
        if(is_string($string) and is_integer($findBy)) {
            ...
            try{
                ...
                switch($xml->Session->StatusInfo->Status) {
                    ...
                    case 'OK':
                        $filename = $this->saveFile("waybills", $xml->DocumentData, $this->department.'_'.$string);
                        if($filename) {
                            $return_array = array(
                                "type" => "ok",
                                "filename" => $filename,
                                "message" => "Etykieta została poprawnie wygenerowana."
                                );
                        } else {
                            $return_array = array(
                                "type" => "error",
                                "message" => "Wystąpił problem z zapisem pliku."
                            );
                        }
                    break;
                }
