...
class Controller_Order extends Controller_Welcome {
	...
	public function action_orders_search() {
		...
		}elseif(!Auth::instance()->logged_in('admin') && Auth::instance()->logged_in('manager')) {
		...
		$orders=ORM::factory('Order')->where('user_id', 'IN', $users_ids);
		$orders_count=ORM::factory('Order')->where('user_id', 'IN', $users_ids);
		
		if($query) $orders = $orders->where_open()->where('id', 'LIKE',"%".$query."%")->or_where('shipping_number', 'LIKE', "%".$query."%")->where_close();
		
		if($query) $orders_count = $orders->where_open()->where('id', 'LIKE',"%".$query."%")->or_where('shipping_number', 'LIKE', "%".$query."%")->where_close();
	...
	public function action_orders_realized() {
		...
		$orders=ORM::factory('Order')->where('user_id', 'IN', $users_ids)->where_open()->and_where('status', '=', 'Dostarczone')->or_where('status', '=', 'Zrealizowane')->where_close()->find_all();		
		//var_dump($orders);
	...
	public function action_add() {
		...
		if($this->request->method()===HTTP_Request::POST) {
			...
			$log->add(Log::ERROR,'Exception Numer 2: '.serialize($_POST)."\n");
			...
			$log->add(Log::ERROR,'Exception Numer 2: '.serialize($params)."\n");
