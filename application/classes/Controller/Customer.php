...
class Controller_Customer extends Controller_Welcome {
	...
	public function before() {
		...
		if(strtolower ( $this->request->action()) == 'users') $this->add_init("TableCustomerUsers.init();\t\n");
    ...
    public function action_user_edit(){   
    	if($this->request->param('id') > 0 ) {
        	...
        	$roles = ORM::factory('Role')->where('name','=','login')->or_where('name', '=', 'manager')->find_all();
    ...
    public function action_user_delete() {
    	if($this->request->param('id') > 0) {
    		...
    		if($user->deleteUser()) {
    			Message::success(ucfirst(__('Użytkownik został usunięty')),'/customer/users/'.$customer_id);
    		}else {
    			Message::error(ucfirst(__('Nie udało się usunąć użytkownika')),'/customer/users/'.$customer_id);
    		}
    	}
    }
