...
class Controller_Virtualbriefcase extends Controller_Welcome {
	...
	public function action_document_add() {
		...
		if($this->request->method()===HTTP_Request::POST) {
			...
			if ($virtualbriefcase->virtualbriefcase->has('documents', $document->document)) {
				Message::error(ucfirst(__('Podany dokument jest już dodany do docelowej teczki.')),'/virtualbriefcase/virtualbriefcases');
			}
	...
	if($this->request->method()===HTTP_Request::POST) {
			...
			if ($virtualbriefcase->virtualbriefcase->has('documentlists', $documentlist->documentlist)) {
				Message::error(ucfirst(__('Podana lista dokumentów jest już dodana do docelowej teczki.')),'/virtualbriefcase/virtualbriefcases');
			}
	...
	public function action_virtualbriefcase_template() {
		$vb_templates = ORM::factory('VirtualBriefcaseTemplate')->find_all();
		$this->content->bind('vb_templates', $vb_templates);
		
		
	}
	
	public function action_virtualbriefcase_template_add_new_virtualbriefcase() {
		$vb_templates = ORM::factory('VirtualBriefcaseTemplate')->find_all();
		$this->content->bind('vb_templates', $vb_templates);
	
		if($this->request->param('id') > 0) {
			Message::success(ucfirst(__('Szablon został wybrany')),'/virtualbriefcase/virtualbriefcase_template_add_new_virtualbriefcase_choose/'.$this->request->param('id'));
		}
	
	}
	
	public function action_virtualbriefcase_template_add_new_virtualbriefcase_choose() {
		if($this->request->param('id') > 0) {
			$vb_template = ORM::factory('VirtualBriefcaseTemplate',$this->request->param('id'));
			$this->content->bind('vb_template', $vb_template);
			...
			if(Auth::instance()->logged_in('admin') || Auth::instance()->logged_in('operator')) {
				...
				$warehouses = ORM::factory('Warehouse')->find_all();
				foreach ($warehouses as $warehouse) {
					array_push($warehouses_ids, $warehouse->id);
				}
		...
		if($this->request->method()===HTTP_Request::POST) {
			...
			$params['division_id'] = $_POST['division_id'];
			
			if($virtualbriefcase->addVirtualBriefcase($params)) {
				foreach ($_POST['doc'] as $doc) {
					$virtualbriefcase->addDocument(array('document_id'=>$doc));
				}
			}
			
			
			Message::success(ucfirst(__('Teczka została dodana')),'/virtualbriefcase/virtualbriefcases');
					
		}
	}
	
	
	public function action_virtualbriefcase_template_add() {
		if($this->request->method()===HTTP_Request::POST) {
			$vb_template = ORM::factory('VirtualBriefcaseTemplate');
			$vb_template->name = $_POST['name'];
			$vb_template->content = serialize($_POST['content']);
			if($vb_template->save()) {
				Message::success(ucfirst(__('Szablon został zapisany')),'/virtualbriefcase/virtualbriefcase_template');
			}else {
				Message::error(ucfirst(__('Nie udało się dodać szablonu')),'/virtualbriefcase/virtualbriefcase_template');
			}
	...
	public function action_virtualbriefcase_template_remove() {
		if($this->request->param('id') > 0) {
			$vb_template = ORM::factory('VirtualBriefcaseTemplate',$this->request->param('id'));
		
			if($vb_template->delete()) {
				Message::success(ucfirst(__('Szablon został usunięty')),'/virtualbriefcase/virtualbriefcase_template');
			}else {
				Message::error(ucfirst(__('Nie udało się usunąć szablonu')),'/virtualbriefcase/virtualbriefcase_template');
			}
		}
