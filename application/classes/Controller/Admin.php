...
class Controller_Admin extends Controller_Welcome {
	...
	public function before() {
		...
		if(strtolower ( $this->request->action()) == 'acls') $this->add_init("TableAcls.init();\t\n");
	...
	protected function load_content() {
		...
		if(strtolower ( $this->request->action()) == 'acls') $this->add_fjs ( ASSETS_ADMIN_PAGES_SCRIPTS.'table-acls.js');
		...
		if (Kohana::find_file ( 'views', $this->_req )) {
			...
			if (file_exists ( DOCROOT.ASSETS_ADMIN_PAGES_SCRIPTS . $this->_req . '.js' )) {
				$this->add_js ( ASSETS_ADMIN_PAGES_SCRIPTS . $this->_req . '.js' );
			}
		}
	}
	...
	public function action_acls() {
		if(Auth::instance()->logged_in('admin')==TRUE) {
			...
			Message::error(ucfirst(__('Brakuje Ci uprawnień synu !')),'/');
		}
			
		if($this->request->method()===HTTP_Request::POST) {
			
			foreach($_POST['acls'] as $acl_id=>$acl_roles) {
				
				
				$a = ORM::factory('Acl',$acl_id);
				
				if($a->loaded()) {
					
					
					foreach($a->roles->find_all() as $role) {
						
					
						if(!in_array($role->id, $acl_roles)) {
				
							$a->remove('roles',$role->id);
						}
					}
					
					foreach ($acl_roles as $role_id) {
						
						if(!$a->has('roles',array('id'=>$role_id))) {
							$a->add('roles', $role_id);
						}
					}	
				}	
			}
			Message::success(ucfirst(__('ACL został dodany')),'/admin/acls');
		}
	}
