...
class Order {
	...
	public function updateHistory($operation="",$last_status="", $currect_status="") {
		
		try {
			$history_record = ORM::factory('OrderHistory');
			$history_record ->order_id = $this->order->id;
			$history_record ->user_id = Auth::instance()->get_user()->id;
			$history_record ->operation=$operation;
			$history_record ->last_status=$last_status;
			$history_record ->current_status=$currect_status;
			$history_record->save();
		}catch (Exception $e) {
	...
	public function updateOrder($params) {
		...
		if($this->order->loaded()) {
			...
			if(date('Y-m-d',strtotime($this->order->pickup_date)) != date('Y-m-d',strtotime($params['pickup_date']))) {
				...
				$this->updateHistory('Date change: '.$params['pickup_date'],$this->order->status, $this->order->status);
	...
	public function deliverOrder() {
		...
		if($this->order->loaded()) {
			...
			$this->updateHistory('Order delivery:',$this->order->status,'Dostarczone');
	...
	public function sendOrderByEmail($params) {
		...
		if($this->order->loaded()) {
			...
			$this->updateHistory('Order finish by email:',$this->order->status,'Zrealizowane');
	...
	public function sendOrder($params) {
		...
		if($this->order->loaded()) {
			...
			$shipmentcompany = $this->order->shipmentcompany;
			
			$this->updateHistory('Order in delivery with '.$shipmentcompany->name,$this->order->status,'W doręczeniu');
			
			$this->status='W doręczeniu';
			$this->order->status='W doręczeniu';

			if($shipmentcompany->name == "DPD") {
				$dpd = DpdWebService::instance();
				
				$dpd->shipToDpd["Company"] = $this->order->user->customer->name;
				$dpd->shipToDpd["Name"] = $this->order->user->firstname;
				$dpd->shipToDpd["Surname"] = $this->order->user->lastname;
				$dpd->shipToDpd["Street"] = $this->order->address->street;
				$dpd->shipToDpd["Number"] = $this->order->address->number;
				$dpd->shipToDpd["City"] = $this->order->address->city;
				$dpd->shipToDpd["CountryCode"] = 'PL';
				$dpd->shipToDpd["PostalCode"] = str_replace('-','',$this->order->address->postal);
				$dpd->shipToDpd["Phone"] = $this->order->address->telephone;
				$dpd->shipToDpd["Phone2"] = '';
				$dpd->shipToDpd["Email"] = $this->order->user->email;
				
				$dpd->packageDetails["package_amount"] = $this->order->boxes->count_all();
				
				$dpd->packageDetails["customer_data_1"] = "PIN: ".$this->order->pin;
				
				foreach ($this->order->boxes->find_all() as $box ) $packageContent .= " ".$box->barcode;
				
				$dpd->packageDetails["package_content"] = $packageContent;
				$dpd->packageDetails["reference_number"] = "#".$this->order->id."-".$this->order->user->id."-".time();
				$dpd->packageDetails["Ref1"] = "#".$this->order->id."-".$this->order->user->id."-".$this->order->pin;
				$dpd->packageDetails["Ref2"] = '';
				$dpd->packageDetails["Ref3"] = '';
				$dpd->packageDetails["COD"] = '0.00';
				$dpd->packageDetails["DeclaredValue"] =  '0.00';
				$dpd->packageDetails["Weight"] = $this->order->boxes->count_all()*11;
				
				
				$dpd->_dpd->setShipTo($dpd->shipToDpd);
				$dpd->_dpd->setPackageDetails($dpd->packageDetails);
				
				$dpd->_dpd->setConnection();
			
				
				$dpd_result = $dpd->_dpd->registerNewPackage();
				
				if($dpd_result['type']=="ok") {
					$label_result = $dpd->_dpd->getLabelPDF(1, $dpd->packageDetails["reference_number"]);
					$this->order->shipping_number=$dpd_result['array']['package_id'];
										
					if($label_result['type']=="ok") {
						$this->order->shipping_label=$label_result['filename']; 
						
						$protocol_result = $dpd->_dpd->getProtocol(array($dpd->packageDetails["reference_number"]));
						
						if($protocol_result['type']=="ok") {	
							$this->order->shipping_protocol=$protocol_result['filename'];
						}elseif($protocol_result['type']=="error") {
							Message::error(ucfirst(__('Wystąpił błąd: ').$protocol_result['message']),'/order/send/'.$this->order->id);
						}
					}elseif($label_result['type']=="error") {
						Message::error(ucfirst(__('Wystąpił błąd: ').$label_result['message']),'/order/send/'.$this->order->id);
					}	
					
				}elseif($dpd_result['type']=="error") {
					
					Message::error(ucfirst(__('Wystąpił błąd: ').$dpd_result['message']),'/order/send/'.$this->order->id);
	...
	public function customerdeliveryOrder() {
		...
		if($this->order->loaded()) {
			...
			$this->updateHistory('Order reciept by customer',$this->order->status,'W trakcie odbioru');
	...
	public function completeOrder() {
		...
		if($this->order->loaded()) {
			if($this->type== 'Zamówienie zniszczenie magazynowanych pudeł') {
				$this->updateHistory('Order completed',$this->order->status,'Zrealizowane');
				$this->status='Zrealizowane';
				$this->order->status='Zrealizowane';
			}elseif($this->type == 'Zamówienie odbioru i magazynowania pudeł') {
				$this->updateHistory('Order completed',$this->order->status,'Zrealizowane');
				$this->status='W dostarczeniu na magazyn';
				$this->order->status='W dostarczeniu na magazyn';
			}else {
				$this->updateHistory('Order awaits sending',$this->order->status,'Oczekuje na wysłanie');
	...
	public function acceptOrder() {
		...
		if($this->order->loaded() && $this->order->status=='Przyjęte do realizacji') {
			return true;
		}
		
		if($this->order->loaded() && $this->order->status !='Przyjęte do realizacji') {
			$this->updateHistory('Order accepted',$this->order->status,'Przyjęte do realizacji');
			...
			if($this->order->update()) {
				...
				if($this->order->type == "Zamówienie zniszczenie magazynowanych pudeł") {
					...
					try {
						...
						$log->add(Log::ERROR,'Exception: '.serialize($user));
						...
						$log->add(Log::ERROR,'Exception: Wystąpił błąd podczas dodwania dokumentu zamówienia'."\n");
						$log->add(Log::ERROR,'Exception: '.$e->getLine());
						$log->add(Log::ERROR,'Exception: '.$e->getMessage());
						$log->add(Log::ERROR,'Exception: '.$e->getTraceAsString());
					}
	...
	public function deleteOrder() {
		...
		if($this->order->loaded()) {
			...
			$this->updateHistory('Order deleted',$this->order->status,'Usunięte');
	...
	public function receptOrder($params) {
		if($this->order->loaded()) {
			...
			$this->updateHistory('Order received by warehouse',$this->order->status,'Na stanie magazynu');
			...
			if($this->order->update()) {
				...
				if($this->order->boxes->count_all() > 0) {
					foreach($this->order->boxes->find_all() as $order_box) {
						$order_box->lock = 0;
						$order_box->utilisation_status=0;
						$order_box->status = "Na magazynie";
						try{
							$order_box->update();
						}catch (Exception $e) {
							$log->add(Log::ERROR,'Exception:'.$e->getMessage()."\n");
		...
		if(is_array($params)) {
			...
			if($params['order_type']==0) {
				...
				$this->order->quantity=$params['box_quantity_1'];
					...
					$this->order->description=$params['order_description_3'];
			}elseif($params['order_type']==4) {
					$this->order->description=$params['order_description_4'];
			}elseif($params['order_type']==5) {
				$this->order->quantity=$params['box_quantity_5'];
			}elseif($params['order_type']==6) {
				$this->order->description=$params['order_description_6'];
			}else {
			...
			try {
				...
				if($this->order->save()) {
					$this->id=$this->order->id;
					$this->order->reload();
					$this->updateHistory('Order created','Nowe','Nowe');
					...
					if($params['order_type'] == 2 || $params['order_type'] == 3 || $params['order_type'] == 4 || $params['order_type'] == 5 || $params['order_type'] == 6) {
						...
						if($params['order_type'] == 2) {
							...
							$document_template = View_MPDF::factory('order/order_document_normal');
						...
						if(is_array($params['box_id'])) { 
							foreach($params['box_id'] as $key=>$value) {
								$order_detail = ORM::factory('OrderDetail');
								$order_detail->box_number = $params['box_id'][$key];
								$order_detail->box_storagecategory = $params['box_storagecategory'][$key]["storagecategory"];
								$order_detail->box_description = $params['box_description'][$key]["description"];
								$order_detail->box_date =  date('Y-m-d', strtotime(date('d').'-'.date('m').'-'.$params['box_end_date'][$key]["box_end_date"])); 
