...
class Box{
	...
	public function receptBox($params) {
		$log=Kohana_Log::instance();
		
		$box = ORM::factory('Box')->where('barcode','=',$params['box_code'])->find();
					
		$box->lock=0;
		$box->seal1=$params['box_seal1'];
		$box->seal2=$params['box_seal2'];
		$box->status = "Na magazynie";
		
		$place = ORM::factory('Place')->where('barcode','=', $params['box_position'])->find();
		
		if($place->loaded()) {
			$box->place_id=$place->id;
		}else {
			$box->place_id=1;
		}
		...
		try {
			if($box->loaded()) {
				...
				$log->add(Log::DEBUG,"Box updated");
				
				if($box->update()) return true;
				else return false;			
			}else {
				$log->add(Log::DEBUG,"Box saved");
				$box->lock=0;
				$box->utilisation_status=0;
				$box->barcode=$params['box_code'];
				$box->date_from = date('Y-m-d');
				$box->date_reception = date('Y-m-d');
				if($box->save()) return true;
				else return false;
			}
		}catch (Exception $e) {
    			$log->add(Log::ERROR,'Exception:'.$e->getMessage()."\n");
	...
	public function changeStatus($params) {
		...
		$box = ORM::factory('Box')->where('barcode','=',$params['box_id'])->find();
				
		try {
			if($box->loaded()) {
				$box->status='Zdjęte z pozycji';	
				$log->add(Log::DEBUG,"Box updated");
