		...
		if(is_array($params)) {
			...
			try {
				...
				if($this->document->update()) {
					...
					try {
						Solr::instance()->index(array('id'=>"document".$this->document->id,'archidox_id'=>$this->document->id,'type'=>'document','name'=>$this->document->name,'description'=>$this->document->description));
					}catch (Exception $e) {
