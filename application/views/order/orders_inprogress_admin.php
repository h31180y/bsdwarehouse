...
<?php if($order->shipmentcompany->name == 'DPD'):?>
												<a target="_blank" href="<?php echo str_replace(array(DIRECTORY_SEPARATOR,"#"),array("/","%23"),Kohana::$base_url.str_replace(DOCROOT,"",Kohana::$config->load('dpd')->files).$order->shipping_label);?>" class="btn btn-default btn-xs"><i class="icon icon-rocket"></i> Etykiety</a>
												<a target="_blank" href="<?php echo str_replace(array(DIRECTORY_SEPARATOR,"#"),array("/","%23"),Kohana::$base_url.str_replace(DOCROOT,"",Kohana::$config->load('dpd')->files).$order->shipping_protocol);?>" class="btn btn-default btn-xs"><i class="icon icon-paper-plane"></i> Protokół</a>
<?php endif;?>
<?php endif;?>
									</div>
								</td>
