		...
		<form action="/order/edit_order/<?php echo $order->id; ?>" class="form-horizontal" id="edit_order_form"
			...
			<div class="tab-content">
				...
				<div id="tab_1-1" class="tab-pane active">
					...
					<div class="portlet-body form">
						<h3 class="block">Kurier</h3>
						<div class="form-group">
							<label class="control-label col-md-3">Firma kurierska</label>
							<div class="col-md-4">
								<?php echo $order->order->shipmentcompany->name; ?>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-md-3">Numer paczki</label>
							<div class="col-md-4">
								<?php echo $order->order->shipping_number; ?>
							</div>
						</div>
						
						
						<?php if($order->order->shipmentcompany->name=="DPD"): ?>
						<div class="form-group">
							<label class="control-label col-md-3">Naklejki na paczkę</label>
							<div class="col-md-4">
								<a target="_blank" href="<?php echo str_replace(array(DIRECTORY_SEPARATOR,"#"),array("/","%23"),Kohana::$base_url.str_replace(DOCROOT,"",Kohana::$config->load('dpd')->files).$order->order->shipping_label);?>" class="btn btn-default"><i class="icon icon-rocket"></i> Pobierz</a>
							</div>
						</div>
						
						<div class="form-group">
							<label class="control-label col-md-3">Protokół kuriera</label>
							<div class="col-md-4">
								<a target="_blank" href="<?php echo str_replace(array(DIRECTORY_SEPARATOR,"#"),array("/","%23"),Kohana::$base_url.str_replace(DOCROOT,"",Kohana::$config->load('dpd')->files).$order->order->shipping_protocol);?>" class="btn btn-default"><i class="icon icon-paper-plane"></i> Pobierz</a>
							</div>
						</div>
					...
					<?php if(Auth::instance()->logged_in('admin')):?>
						<br />
						<?php echo View::factory('order/order_history')->bind('order',$order)->render();?>						
					<?php endif;?>
